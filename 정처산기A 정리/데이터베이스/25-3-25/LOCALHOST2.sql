...
--서브쿼리
select rownum as RN, usertbl. * from usertbl;   --프로그램에서 자체적으로 부여하는 
                                                --열 숫자를 이용하여 정렬
select * from
(select rownum as RN, usertbl. * from usertbl); 
--가상 숫자이기 때문에 이를 이용해서 특정 행을 선택하는 등을 하면 오류가 생긴다
--select부터 from까지 괄호로 감싸면 테이블로 간주한다


--group 문제

--1. userid에서 지역별 사용자 수를 구하시오
select addr, count (*) as 인원수
from usertbl
group by addr;

--2. buytbl에서 사용자 별 총 구매액을 구하시오
select userid, sum(price*amount) as 총구매액
from buytbl
group by userid;

--3. buytbl에서 제품 그룹별 판매 수량의 합계를 구하시오 null은 미분류로 표시
select groupname, sum (amount)
from buytbl  where amount is not null
group by groupname;

select coalesce(groupname,'미분류'), sum (amount)
from buytbl  where amount is not null
group by groupname;
--coalesce 함수는 null값을 선별하는 함수

select nvl(groupname,'미분류'), sum (amount)
from buytbl  where amount is not null
group by groupname;
--nvl 함수도 null값을 선별하는 함수이다

--4. usertbl에서 출생년도별 평균 키를 구하시오
select birthyear, avg (height)
from usertbl
group by birthyear;

--5. buytbl에서 제품명별 구매 횟수와 총 구매액을 구하고 내림차순으로 정렬
select prodname, count (*) as 구매횟수, sum(price*amount) as 총구매액
from buytbl
group by prodname
order by 구매횟수 desc;
--큰따옴표나 따옴표를 쓰지 않을 땐 식별자로 인식
--작은 따옴표는 식으로 인식

--6. usertbl에서 통신사별 가입자 수를 구하고, null값은 미입력으로 표시
select nvl(mobile1,'미입력'), count (*)
from usertbl
group by mobile1;

--7. usertbl과 buytbl을 조인하여 지역별 총 구매액을 구하시오


--8. buytbl에서 사용자별 구매한 제품의 종류 수를 구하시오
select * from buytbl;
select userid, count ( distinct prodname)   --distinct는 중복 값 없애는 것
from buytbl
group by userid;
--groupe by 는 꼭 하나만 잡지 않아도 된다 여러개도 잡을 수 있다

--9. usertbl에서 가입 연도별 가입자 수를 구하시오
select * from usertbl;
--select mdate, count(name)
--from usertbl
--group by mdate;
select extract (year from mdate), count(*)
from usertbl
...
--10. buytbl과 usertbl을 조인하여 연령대별 구매 총액을 계산. 2025년 기준으로 연령 계산
