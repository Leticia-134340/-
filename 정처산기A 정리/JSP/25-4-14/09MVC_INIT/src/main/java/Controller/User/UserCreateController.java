...
public class UserCreateController implements SubController {
	...
	private boolean isValid(UserDto userDto) {
		if (userDto.getUsername() == null || userDto.getUsername().length() <= 4) {
			req.setAttribute("username_err", "username의 길이는 최소 5자 이상이어야 합니다");
			System.out.println("[INVALID] username의 길이는 최소 5자 이상이어야 합니다");
			return false;
		}
		if (userDto.getUsername().matches("^[0-9].*")) {
			System.out.println("[INVALID] username의 첫문자로 숫자가 들어올 수 없습니다");
			req.setAttribute("username_err", "username의 첫문자로 숫자가 들어올 수 없습니다");
			return false;
		}
