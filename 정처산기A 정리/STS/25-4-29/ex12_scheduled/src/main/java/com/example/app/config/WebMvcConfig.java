...
public class WebMvcConfig implements WebMvcConfigurer{
	...
	//1) EAN MAPPING(BeanNameUrlHandlerMapping : 요청 URL을 동일한 이름을 가진 Bean에 매핑
	@Bean
	BeanNameUrlHandlerMapping beanManeUrlHandlerMapping() {
	...
	//2) SimplerUrlHandlerMapping : 개발자가 직접 매핑정보를 설정 방식 중 하나, 깁노값 : 정적자원 매핑
	@Bean
	...
	RequestMappingHandlerMapping requestMappingHandlerMapping() throws NoSuchMethodException, SecurityException {
		...
		//URL 요청 매핑정보 구성
		RequestMappingInfo mappinfInfo = RequestMappingInfo
											.paths("/custom_03")
		...
		//요청 매핑정보, 핸들러, 매서드 등록
		handlerMapping.registerMapping(mappinfInfo, new CustomHandler(), method);
